...
### 1. 점수 병목 진단 (자동 도출)
클래스별 F1을 보면 대부분의 클래스는 F1 0.9~1.0으로 쉽게 분리되지만,
소수의 클래스 집합이 상호 혼동되며 Macro-F1을 지배합니다. 원본 데이터에서는
**{0, 3, 9, 15, 19}**가 F1 0.2~0.47로 서로 섞였습니다.

이 집합을 코드에 적어 두는 대신 검증 예측의 혼동행렬에서 찾아냅니다.
클래스별 F1 하위 분위를 후보로 두고, 상호 오분류율이 임계 이상인 클래스들을
간선으로 이어 가장 큰 연결요소를 클러스터로 삼습니다(`src/discovery.py`).
서로 혼동되는 집합이 없으면 빈 결과를 돌려주고 전문가 모델을 자동으로 끕니다.

### 2. FDI 잔차 피처 (analytical redundancy)
고장 진단(Fault Detection & Isolation) 문헌의 고전적 접근을 적용했습니다.
데이터에 물리적으로 중복 계측된 센서쌍이 다수 존재합니다. 원본 데이터에서는
완전 중복(corr = 1.0) `X_06=X_45`, `X_10=X_17` 과 준중복(|corr| > 0.95) 12쌍이
관측됐습니다.

이 목록도 하드코딩하지 않고 train 상관행렬에서 매 실행 도출합니다.
완전 중복은 union-find로 묶어 그룹마다 대표 하나만 남기고(잔차가 항상 0이므로
잔차 피처 후보에서도 제외), 준중복은 상관이 강한 순으로 상한까지 골라
**표준화 잔차 피처**를 만듭니다. 정상 상태에선 잔차 ≈ 0, 센서 드리프트/편향
고장에선 잔차가 벌어지는 시그니처를 이용합니다. 홀드아웃 검증에서
Macro-F1 **0.7942 → 0.8017 (+0.0075)** 개선을 확인했습니다.

### 3. 하드클러스터 2단계 리랭킹
전역 모델이 하드클러스터 중 하나로 예측한 샘플에 대해, 그 클래스들만으로
...
파이프라인이 쓰는 데이터 의존적 상수는 모두 `src/discovery.py`가 매 실행 도출합니다.
`AUTO_DISCOVER=False`로 끄면 `src/features.py`의 폴백 상수를 씁니다.
도출 결과는 `metrics.json`의 `discovery` 블록에 그대로 기록됩니다.

| 값 | 도출 방법 | 라벨 사용 |
|---|---|---|
| 완전 중복 센서 그룹 | train 상관행렬에서 \|corr\| ≥ `EXACT_DUP_THRESHOLD` union-find 그룹화, 대표 하나만 유지 | 없음 |
| 준중복 센서쌍 | \|corr\| ≥ `CORR_THRESHOLD` 인 쌍을 강한 순으로 `MAX_REDUNDANT_PAIRS` 개까지 | 없음 |
| 분포 shift 컬럼 | 참조(test) 분위수 구간에서 그 폭의 `SHIFT_EXCESS_RATIO` 배 이상 벗어난 값이 있고, 그런 행이 `SHIFT_OUTSIDE_FRAC` 이하인 컬럼 | 없음 |
| 분산 시그니처 컬럼 | 클래스별 std의 변동계수 상위 `N_SIGNATURE_COLS` 개 | train 라벨 |
| 하드클러스터 | 검증 예측 혼동행렬에서 저성능 클래스들의 상호 오분류 연결요소 | 검증 라벨 |

shift 탐지는 경계를 살짝 넘는 값을 세지 않습니다. 참조 표본이 train보다 작으면
정상적인 표본 변동만으로도 범위 밖 행이 생기기 때문입니다. 참조 폭의 배수만큼
멀리 떨어진 값만 꼬리로 봅니다. 또 수십 개 컬럼을 동시에 보면 우연히 임계를 넘는
컬럼이 나올 수 있으므로, 선택되지 않은 컬럼 중 최고 비율(`shift_runner_up_ratio`)을
함께 기록해 실제로 도드라지는 값인지 확인할 수 있게 했습니다.

하드클러스터는 각 fold의 내부 홀드아웃 예측을 모아 도출합니다. 바깥 fold의 OOF를
쓰지 않으므로, 클러스터 선택이 OOF 점수에 직접 들어가지 않습니다. 내부 홀드아웃을
만들지 못해 폴백한 실행에서는 `metrics.json`의 `hard_cluster_source`가
`outer_fold`가 되며, 그 경우 클러스터 선택분만큼 OOF가 낙관적입니다.
...
│   ├── discovery.py     # 센서쌍/shift 컬럼/시그니처/하드클러스터 자동 도출
...
- 자동 도출이 심어 둔 구조(중복 센서, 극단 꼬리, 클래스별 산포, 혼동 클러스터)를 찾아내고,
  구조가 없는 데이터에서는 아무것도 지어내지 않는지
...
| `CLIP_SHIFT_COLS` | True | 탐지된 shift 컬럼을 참조 분위수로 클리핑 |
| `AUTO_DISCOVER` | True | 센서쌍/shift/시그니처 컬럼 자동 도출 (False=폴백 상수) |
| `CORR_THRESHOLD` | 0.95 | 준중복 센서쌍 기준 \|corr\| |
| `HARD_CLUSTER` | "auto" | 혼동행렬에서 도출. 리스트를 주면 고정 |
...
- shift 컬럼 클리핑만 test 피처 분포(분위수)를 참조하는 transductive 변환입니다.
  라벨은 보지 않으며, 클리핑을 끄면 참조 프레임이 파이프라인에 전혀 영향을 주지
  않는다는 점을 테스트로 고정했습니다.
- 하드클러스터는 fold 내부 홀드아웃 예측에서 도출하므로 바깥 fold OOF와 분리됩니다.
